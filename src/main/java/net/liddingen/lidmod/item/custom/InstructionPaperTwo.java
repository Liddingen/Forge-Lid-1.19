...
public class InstructionPaperTwo extends Item {
    public InstructionPaperTwo(Properties properties) {
        super(properties);
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag flag) {
        if (Screen.hasShiftDown()) {
            components.add(Component.literal("go deep in the caves and find nine music disc fragments" +
                    " to craft the 5th music disc. Then bring the disk to me!").withStyle(ChatFormatting.AQUA));
        } else {
            components.add(Component.literal("Press SHIFT for more info").withStyle(ChatFormatting.YELLOW));
        }
    }

    private void outputRandomInstructionTwo(Player player) {
        player.sendSystemMessage(Component.literal("go deep in the caves and find nine musik disc fragments" +
                " to craft the 5th musik disc. Then bring the disk to me!").withStyle(ChatFormatting.GOLD));
    }
}

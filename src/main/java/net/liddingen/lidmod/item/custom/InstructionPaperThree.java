...
public class InstructionPaperThree extends Item {
    public InstructionPaperThree(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (!level.isClientSide() && hand == InteractionHand.MAIN_HAND) {
            outputRandomInstructionThree(player);
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level p_41422_, List<Component> components, TooltipFlag flag) {
        if (Screen.hasShiftDown()) {
            components.add(Component.literal("Lately it got a bit lonely in here... Go out in the ocean" +
                    " to catch me an axolotl in a bucket so it can be my new friend!").withStyle(ChatFormatting.AQUA));
        } else {
            components.add(Component.literal("Press SHIFT for more info").withStyle(ChatFormatting.YELLOW));
        }
    }

    private void outputRandomInstructionThree(Player player) {
        player.sendSystemMessage(Component.literal("Lately it got a bit lonely in here... Go out in the ocean" +
                " to catch me an axolotl in a bucket so it can be my new friend!").withStyle(ChatFormatting.GOLD));
    }
}
